...
mod libc {
    pub use crate::libc_consts::*;
}

mod tcp {
    pub use crate::arceos_tcp::*;
}

const FAKE_SOCKET_FD: libc::c_int = 1234;
...
use libc::{AsRawFd, FromRawFd, IntoRawFd};
#[cfg(feature = "all")]
...
pub(crate) fn bind(fd: &Arc<AxTcpSocketHandle>, addr: &SocketAddr) -> io::Result<()> {
...
pub(crate) fn listen(fd: &Arc<AxTcpSocketHandle>, backlog: c_int) -> io::Result<()> {
    ...
    tcp::listen(fd, backlog as u32)
}

pub(crate) fn accept(fd: &Arc<AxTcpSocketHandle>) -> io::Result<(Arc<AxTcpSocketHandle>, SocketAddr)> {
...
pub(crate) fn recv(fd: &Arc<AxTcpSocketHandle>, buf: &mut [MaybeUninit<u8>], _flags: c_int) -> io::Result<usize> {
    ...
    .map(|n| n as usize)
    */
    unsafe { tcp::recv(fd, MaybeUninit::slice_assume_init_mut(buf)) }
}
...
pub(crate) fn send(fd: &Arc<AxTcpSocketHandle>, buf: &[u8], _flags: c_int) -> io::Result<usize> {
    /*
    syscall!(send(
        fd,
        buf.as_ptr().cast(),
